from django.http import HttpResponse, HttpResponseRedirect, HttpResponsePermanentRedirect, HttpResponseForbidden, JsonResponse, HttpResponseNotFound
...
def user(request):
    login= request.GET.get("login","Undefiend")
...
def error(request):
    return HttpResponseNotFound(f"Not found", status=404, reason="Error")

def access(request,name,password):
    if (name=='admin'and password=='admin'):
        return HttpResponse("верно")
    else:
        return HttpResponseForbidden("неверно")

def json(request):
    return JsonResponse({"name":"artyom","age":18})
def set(request):
    username=request.GET.get("username","Ezil")
